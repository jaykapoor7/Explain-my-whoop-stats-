    ...
    <div className="min-h-screen bg-ink-950 text-ink-50">
      ...
      <section className="relative overflow-hidden">
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
          <div className="absolute left-1/2 top-[-8rem] h-[34rem] w-[60rem] -translate-x-1/2 rounded-full bg-recovery/[0.11] blur-[110px]" />
          <div
            className="absolute inset-0 opacity-[0.5]"
            style={{
              backgroundImage: "radial-gradient(rgba(59,46,20,0.07) 1px, transparent 1px)",
              backgroundSize: "24px 24px",
              maskImage: "radial-gradient(130% 85% at 50% 0%, #000 42%, transparent 82%)",
              WebkitMaskImage: "radial-gradient(130% 85% at 50% 0%, #000 42%, transparent 82%)",
            }}
          />
        </div>

        {/* Centered hero */}
        <div className="mx-auto max-w-3xl px-6 pt-16 text-center lg:pt-24">
          ...
          <h1 className="mx-auto mt-6 max-w-3xl font-display text-[2.85rem] font-bold leading-[0.98] tracking-[-0.035em] text-ink-50 sm:text-[4rem]">
          ...
          <p className="mt-6 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs text-ink-400">
        ...
        {/* Wide product dashboard — the shipped Today view, framed and centered. */}
        <div className="mx-auto mt-14 max-w-5xl px-6 pb-14">
          <AppWindow tab="today">
            <div className="flex items-center justify-between border-b border-black/[0.06] pb-4">
              ...
              <span className="rounded-full bg-recovery/15 px-3 py-1 text-[11px] font-semibold text-recovery">Adequate · in line with baseline</span>
            </div>

            {/* Three rings across the top */}
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {[
                { score: 72, color: "#13b57e", label: "Recovery", sub: "Adequate" },
                { score: 65, color: "#eb9d18", label: "Energy", sub: "Charged" },
                { score: 88, color: "#7b68ee", label: "Sleep", sub: "8h 02m" },
              ].map((r) => (
                <div key={r.label} className="flex flex-col items-center rounded-2xl border border-black/[0.06] bg-black/[0.015] px-3 py-5">
                  <Ring score={r.score} color={r.color} label={r.label} size={112} />
                  <span className="mt-2 text-[11px] text-ink-400">{r.sub}</span>
                </div>
              ))}
            </div>

            {/* Strain slim row */}
            <div className="mt-3 flex items-center gap-3 rounded-2xl border border-black/[0.06] bg-black/[0.015] px-4 py-3">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#ef5a45]/15 text-[#ef5a45] text-sm font-bold">🔥</span>
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#ef5a45]">Strain</div>
                <div className="text-[11px] text-ink-400">Threshold run · zone 4</div>
              </div>
              <span className="tabular ml-auto font-display text-lg font-bold text-ink-50">11.4<span className="text-[11px] text-ink-400">/21</span></span>
            </div>

            {/* Ledger, two columns to fill the width */}
            <div className="mt-5">
              <div className="mb-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-400">What drove it today</div>
              <div className="grid gap-x-8 gap-y-1.5 sm:grid-cols-2">
                {LEDGER.map((r) => (
                  <div key={r.label} className="flex items-center gap-3">
                    ...
                    <div className="ml-auto h-1.5 w-16 overflow-hidden rounded-full bg-black/[0.06]">
